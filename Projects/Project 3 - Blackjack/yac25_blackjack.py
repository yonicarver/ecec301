...
class FaceCard(Card):
    ...
    def __init__(self, face, suit):
        """ hard & soft vals of face cards is 10 """
        self._face = face                           # var for face (J, Q, K)
        super(FaceCard, self).__init__(10, suit)    # hard & soft val == 10
    ...
    def unicode(self):
        ...
        suit and the card rank
        """
        return "%s %s" % (self._suit, self._face)
    #######################################################################
...
class AceCard(Card):
    ...
    def __init__(self, suit):
        """ soft val = 11, hard val = 1 """
        super(AceCard, self).__init__(1, suit)      # hard val passed through init function

    def softValue(self):
        """ assign soft value = 11 """
        return 11

    def unicode(self):
        """ display unicode symbol for suit """
...
class CardContainer(object):
    ...
    def popCard(self):
        ...
        """
        ###################################################################
        return self._cards.pop(0)       # pop the first card in the deck
        ###################################################################
    ...
    def pushCard(self, card):
        ...
        """
        ###################################################################
        self._cards.append(card)        # add card to deck
        ###################################################################
...
class Deck(CardContainer):
    ...
    def _loadDeck(self):
        ...
        """
        ###################################################################
        for suit in suits:              # create cards with different suits
            for num in range(2, 11):    # create cards with different numbers
                self.pushCard(Card(num, suit))      # add number card to deck
            for face in face_cards:     # create cards with different faces
                self.pushCard(FaceCard(face, suit)) # add face card to deck
            self.pushCard(AceCard(suit))    # add ace card to deck
        ###################################################################
    ...
    def dealCard(self):
        ...
        """
        ###################################################################
        if len(self._cards) == 0:   # if the deck is empty
            self._loadDeck()        # load the deck with cards
            self.shuffle()          # shuffle the deck

        return self.popCard()       # return card to enter play
        ###################################################################
...
class Hand(CardContainer):
    ...
    def getHoleCard(self):
        ...
        """
        ###################################################################
        return self._holeCard       # return value of hole card
        ###################################################################
    ...
    def clear(self):
        ...
        """
        ###################################################################
        self._cards = []        # clears deck
        self._holeCard = None   # clears hole card
        ###################################################################
    ...
    def getCard(self, card):
        ...
        """
        ###################################################################
        if self._holeCard == None:      # if the hand is empty...
            self._holeCard = card       # move card to hole
        else:                           # if there are cards in the hand...
            self.pushCard(card)         # show card
        ###################################################################
    ...
    def getSoftScore(self):
        ...
        for card in self:       # for each card in hand...
            score += card.softValue()        # add soft value to score
        score += self._holeCard.softValue()     # add hole card value to score
    ...
    def getHardScore(self):
        ...
        score = 0       # initialize score to 0
        for card in self:       # for each card in hand...
            score += card.hardValue()        # add hard value to score
        score += self._holeCard.hardValue()     # add hole card value to score
...
class Game(object):
    ...
    def _checkHouseBust(self):
        ...
        """
        ###################################################################
        return self._house.getHardScore() > 21
        ###################################################################
    ...
    def _doHouseAi(self):
        ...
        """
        ###################################################################
        soft_score = self._house.getSoftScore()

        finalscore = 0
        if soft_score <= 21:
            finalscore = soft_score
        else:
            finalscore = self._house.getHardScore()

        while self._checkPlayerBust() == True:
            if finalscore >= 18:
                break
    ...
    def _checkWhoWins(self):
        ...
        """
        ###################################################################
        self._revealHouseHand()     # show house's hand

        playhard = self._player.getHardScore()
        playsoft = self._player.getSoftScore()
        ...
        househard = self._house.getHardScore()
        housesoft = self._house.getSoftScore()

        if housesoft <= 21:
            housescore = housesoft
        else:
            housescore = househard
...
#       -- If the scores are tied, you (the Player) wins.
    ...
    def playHand(self):
        ...
        while True:
            self._printPlayer()
            options = self._promptChoices('[h]it or [s]tay: ', ['h', 's'])
            if options is 'h':
